...
VID_FTDI = 0x0403
port_device = (
    ""  # This is the device name to the serial port, such as 'COM3' or '/dev/ttyUSB0'.
)
...
def print_credit(pa):
    print("+-" * 40)
    print(
        "SDI-12 Sensor and Analog Sensor Python Data Logger with Telemetry V", version
    )
    print(
    ...
    print(
        "\nVisit https://thingspeak.com/channels/%s to inspect or retrieve data"
        % (pa["channelID"])
    )
...
def interactive_session(pa):
    ...
    for w in a:
        ...
        )  # Windows OS FTDI driver adds 'A' to the end of the serial number.
    ...
    port_device = a[
        int(user_port_selection)
    ].device  # Store the device name to later open port with.

    # Store serial port ID
    pa["ser"].append(
        a[int(user_port_selection)].serial_number
        if platform.system() != "Windows"
        else a[int(user_port_selection)].serial_number[:-1]
    )  # Windows OS FTDI driver adds 'A' to the end of the serial number.
    ...
    time.sleep(
        2.5
    )  # delay for arduino bootloader and the 1 second delay of the adapter.
    ...
    pa[
        "sdi_12_address"
    ] = user_sdi_12_address.strip()  # Remove any \r from an input file typed in windows
...
def sensor_info(pa):
    for an_address in pa["sdi_12_address"]:
        ...
        print(
            ...
            an_address,
            " Sensor info:",
            sdi_12_line.decode("utf-8").strip(),
        )
...
if (
    fnf
):  # No configuration file. Start interactive session. Serial port is open in the interactive session.
    ...
    try:  # Open port by ID
        ...
        for w in a:
            if (
                w.serial_number.__str__()[: len(paras["ser"][0])] == paras["ser"][0]
            ):  # Match ID with the correct port
...
if paras["time_zone_choice"] == 0:
    ...
    )  # use local time, not recommended for multiple data loggers in different time zones

data_file_name = "%s_%04d%02d%02d.csv" % (
    config_file_name,
    now.year,
    now.month,
    now.day,
...
data_file = open(
    data_file_name, "a"
)  # open config_file_name_yyyymmdd.csv for appending
...
for j in range(paras["total_data_count"]):
    ...
    output_str = "%04d/%02d/%02d %02d:%02d:%02d%s" % (
        now.year,
        now.month,
        now.day,
        now.hour,
        now.minute,
        now.second,
        " GMT" if paras["time_zone_choice"] == 0 else "",
    ...
    for cmd_ptr, an_address in enumerate(paras["sdi_12_address"]):
        ...
        for a_command in paras["sdi_12_command"][cmd_ptr]:
            try:
                if a_command == "0":
                    ...
                    complete_command = (
                        an_address.encode() + b"M" + a_command.encode() + b"!"
                    )
                ser[ser_ptr].write(complete_command)
                ...
                if (
                    sdi_12_line == b""
                ):  # Didn't get a response from the sensor? Faulty wiring?
                    print(
                        "Sensor %s failed to respond to command %s."
                        % (an_address, complete_command)
                    )
                    no_data = True  # End the current iteration of sensors and commands on each sensor and wait for the next iteration.
                    break
                sdi_12_line = sdi_12_line[
                    :-2
                ]  # remove \r and \n since [0-9]$ has trouble with \r
                m = re.search(
                    b"[0-9]$", sdi_12_line
                )  # This should match a number ([0-9]) that appears at the end of the response ($), which is a 1-digit number of "returned values" but it is having trouble with the \r so I removed \r\n in the previous line of code.
                if (
                    not m
                ...
                if sdi_12_line != an_address.encode() + b"\r\n":
                    print(
                        "Sensor %s didn't respond with correct service request."
                        % (an_address)
                    )
                    no_data = True  # End the current iteration of sensors and commands on each sensor and wait for the next iteration.
                    break
                ...
                for d_command in range(10):
                    complete_command = (
                        an_address.encode() + b"D" + str(d_command).encode() + b"!"
                    )
        ...
        for value_i in values:
            ...
            if i < max_upload_values:
                value_str = value_str + "&field%d=%s" % (
                    i + 1,
                    value_i,
                )  # format values for posting. Field starts with field1, not field0.
                i = i + 1
    if no_data == True:
        no_data = False
        time.sleep(paras["delay_between_pts"])
    ...
    while (
        i < max_upload_values
    ):  # Pad with zeros in case we don't have max_upload_values fields. This is only necessary for certain servers.
        value_str = value_str + "&field%d=0" % (
            i + 1
        )  # format values for posting. Field starts with field1, not field0.
        i = i + 1
    ...
    http_request_url = http_request_url_format % (
        paras["api_key"],
        value_str,
    )  # Format url command
    ...
    values = (
        []
    )  # clear values for the next iteration, 3.2.3 doesn't support clear as 3.4.3 and 3.5.1 does
